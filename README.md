...
IOU and Dice Scores were used to compare the accuracy of the models. Along with those metrics the training time per epoch and the time it takes to run the model on a single image (inference time) for all the models is also summarized in the below plots.
